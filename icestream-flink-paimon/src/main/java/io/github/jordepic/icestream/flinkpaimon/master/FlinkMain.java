...
public final class FlinkMain {
    ...
    private static void run(Config config) {
        ...
        DeleteConverter converter = buildConverter(config, flink, paimonIndex, remoteFlink);
    ...
    /**
     * Builds the streaming converter: one long-lived warm Flink job whose Paimon lookup cache stays
     * warm across conversions (see {@link StreamingFlinkDeleteFileCreator}), avoiding the
     * per-conversion SST/lookup rebuild that dominates cost. The converter hosts the channel server
     * itself; on a remote cluster the TaskManagers reach it at the configured advertised host:port,
     * in a local MiniCluster over loopback.
     */
    private static DeleteConverter buildConverter(
            Config config, FlinkContext flink, PaimonIndex paimonIndex, boolean remoteFlink) {
