...
/**
 * Shared Flink+Paimon lookup-join wiring used by both the batch ({@link FlinkDeleteFileCreator}) and
 * streaming ({@link StreamingFlinkDeleteFileCreator}) converters: registers the Paimon catalog, the
 * probe types, and turns a stream of probe rows {@code (spec_id, partition_key, pk)} into the lookup
 * matches {@code (spec_id, partition_key, data_file_path, pos)} via {@link LookupJoinSql}.
 */
public final class PaimonLookupJoin {
    ...
    /**
     * Register {@code probeRows} as the eq-delete view and run the lookup join against the index,
     * returning the match rows. {@code useLookupJoin=false} drops the temporal hint for a full-scan
     * regular join — the benchmark control only.
     */
    public static DataStream<Row> lookupMatches(
            ...
            TableIdentifier id,
            boolean useLookupJoin) {
        tEnv.createTemporaryView(LookupJoinSql.EQ_DELETES_VIEW, tEnv.fromDataStream(probeRows, probeViewSchema()));
        String fqn = indexFqn(paimonIndex, id);
        String sql = useLookupJoin ? LookupJoinSql.lookupJoin(fqn) : LookupJoinSql.regularJoin(fqn);
        return tEnv.toDataStream(tEnv.sqlQuery(sql));
    }
}
